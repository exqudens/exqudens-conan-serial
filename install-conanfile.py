...
class ConanConfiguration(ConanFile):
    ...
    options = {"shared": [True, False], "reference": [None, "ANY"], "name": [None, "ANY"], "version": [None, "ANY"]}
    default_options = {"shared": True, "reference": "serial/1.2.1", "name": "serial", "version": "1.2.1"}

    def requirements(self):
        try:
            self.requires(str(self.options.reference))
    ...
    def configure(self):
        try:
            self.options['serial'].shared = self.options.shared
        except Exception as e:
            logging.error(e, exc_info=True)
            raise e
    ...
    def imports(self):
        try:
            self.copy(pattern="*", dst=f"{self.options.name}-{self.options.version}")
        except Exception as e:
            logging.error(e, exc_info=True)
            raise e
